...
def load_correction_results(project_root, input_path=None):
    """加载 correction_results.yaml"""
    path = input_path or os.path.join(project_root, 'result', 'correction_results.yaml')
...
def run(project_root, cfg, input_path=None, output_path=None, table_dir=None):
    """主计算流程"""
    # 尝试加载修正结果
    corr_data = load_correction_results(project_root, input_path=input_path)

    if corr_data is None:
        print(f"未找到 correction_results.yaml: {input_path or os.path.join(project_root, 'result', 'correction_results.yaml')}")
        print("请先运行 correction.py collect，或用 -i 指定已有 correction_results.yaml")
    ...
    # 写入主 YAML
    yaml_path = output_path or os.path.join(project_root, 'result', 'defect_results.yaml')
    result_dir = os.path.dirname(os.path.abspath(yaml_path))
    ...
    # 写入 E_f vs E_F 表格
    ef_dir = table_dir or os.path.join(result_dir, 'E_forms')
...
""",
    ...
    parser.add_argument("-o", "--output", default=None, help="输出 defect_results.yaml。")
    parser.add_argument("--project-root", default=None, help="项目根目录，用于读取 config.yaml、formation.out 和 bulk 信息。")
    parser.add_argument("--config", default=None, help="显式指定 config.yaml。")
    parser.add_argument("--table-dir", default=None, help="E_f vs E_F 表格输出目录。默认与 -o 同目录下的 E_forms。")
    ...
    if args.project_root:
        ...
        project_root = os.path.dirname(parent) if os.path.basename(parent) == "result" else get_project_root(__file__)
    else:
    ...
    run(
        ...
        table_dir=os.path.abspath(args.table_dir) if args.table_dir else None,
    )
