...
class OccupationAnalyzer:
    def __init__(self, input_dir=".", output_dir=".", output_file=None):
        self.nkpt = 0
    ...
    def input_path(self, filename):
        return filename if os.path.isabs(filename) else os.path.join(self.input_dir, filename)

    def output_path(self, filename):
        if self.output_file and filename == 'occ.input':
            return self.output_file
        return filename if os.path.isabs(filename) else os.path.join(self.output_dir, filename)
        
    def read_nkpt(self):
        """读取k点数量"""
        path = self.input_path('OUT.KPT')
        if os.path.exists(path):
            with open(path, 'r') as f:
                self.nkpt = int(f.readline().strip())
        else:
            raise FileNotFoundError(f"OUT.KPT文件不存在: {path}")
    ...
    def generate_occ_input(self, nkpt, defect_info, occ_diff, q_diff, spin_suffix=""):
        ...
        os.makedirs(os.path.dirname(self.output_path(filename)), exist_ok=True)
    ...
    def analyze(self):
        ...
        for f in files:
            if not os.path.exists(self.input_path(f)):
                raise FileNotFoundError(f"{f}文件不存在: {self.input_path(f)}")
...
说明:
  ...
  - 当 -o 是文件路径时，只会把无自旋主 occ.input 写到该文件；其他文件仍写到同目录。
""",
    )
    parser.add_argument("-i", "--input", default=".", help="输入目录，包含 OUT.KPT、OUT.OCC0、OUT.OCC1。")
    parser.add_argument("-o", "--output", default=".", help="输出目录，或无自旋 occ.input 的输出文件。")
    args = parser.parse_args()

    output = os.path.abspath(args.output)
    output_file = output if os.path.splitext(output)[1] else None
    output_dir = os.path.dirname(output) if output_file else output

    analyzer = OccupationAnalyzer(input_dir=args.input, output_dir=output_dir, output_file=output_file)
    analyzer.analyze()
